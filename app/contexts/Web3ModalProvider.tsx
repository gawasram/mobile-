...
const Web3ModalProvider = ({ children }) => {
  ...
  const connect = useCallback(async () => {
    try {
      console.log("Connecting wallet..."); // Log 1
      if (!web3Modal) {
        console.error("Web3Modal not initialized"); // Log Error 1
        return;
      }
  
      const provider = await web3Modal.connect();
      if (!provider) {
        console.error("Provider not found"); // Log Error 2
        return;
      }
      ...
      setChainId(Number(chainId));
      setConnected(true);
      console.log("Wallet connected"); // Log 2
    } catch (error) {
      console.error("Error connecting wallet:", error); // Log Error 3
    }
