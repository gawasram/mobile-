...
import brickToken from './contracts/brick';
import { addressArray } from './constants';

export default class brickTokenWrapper {
    web3: Web3;
    chainId: number;
    account: string;
    wrapperOptions: any;
    ...
    constructor(web3, chainId, account, options = {}) {
        ...
        this.Contract = new brickToken(this.wrapperOptions, addressArray[6].Contract[this.chainId]);
    }
    ...
    async mintBrick(_amount) {
        try {
            const tx = await this.Contract.send('mintBrick', { from: this.account }, _amount);
        } catch (error) {
            throw error;
        }
    }
